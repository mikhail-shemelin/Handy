...
- Install with:
  ...
  # Ubuntu/Debian
  sudo apt update
  sudo apt install build-essential libasound2-dev libpipewire-0.3-dev pkg-config libssl-dev libvulkan-dev vulkan-tools glslc spirv-headers glslang-tools libgtk-3-dev libwebkit2gtk-4.1-dev libayatana-appindicator3-dev librsvg2-dev libgtk-layer-shell0 libgtk-layer-shell-dev patchelf cmake

  # Fedora/RHEL
  sudo dnf groupinstall "Development Tools"
  sudo dnf install alsa-lib-devel pipewire-devel pkgconf openssl-devel vulkan-devel \
  ...
  # Arch Linux
  sudo pacman -S base-devel alsa-lib pipewire pkgconf openssl vulkan-devel \
...
```bash
git clone git@github.com:mikhail-shemelin/Handy.git
cd Handy
```
...
The raw binary (`src-tauri/target/release/handy-hybrid`) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.

**Install from the deb bundle** (works on any Linux distro):

```bash
cd /tmp
ar x /path/to/Handy/src-tauri/target/release/bundle/deb/Handy\ Hybrid_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/handy-hybrid /usr/bin/
sudo cp -a usr/lib/. /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp "usr/share/applications/Handy Hybrid.desktop" /usr/share/applications/
```

The runtime libraries live in the app-private `/usr/lib/handy-hybrid/` (on the binary's rpath), so no `ldconfig` step is needed.

After subsequent rebuilds, copy the binary and any refreshed runtime libraries:

```bash
sudo cp src-tauri/target/release/handy-hybrid /usr/bin/
sudo mkdir -p /usr/lib/handy-hybrid
sudo cp -a src-tauri/transcribe-libs/. /usr/lib/handy-hybrid/
```
...
If the build compiles all the way to `Built application at: ...\handy-hybrid.exe` and
then fails with:

```
Signing C:\...\handy-hybrid.exe with a custom signing command
