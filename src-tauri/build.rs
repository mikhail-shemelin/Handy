fn main() {
    ...
    // `/usr/lib/handy-hybrid` (the dir tauri already uses for resources; keeps
    // Handy Hybrid's libs out of the ldconfig-scanned `/usr/lib`, issue #1639) while
    ...
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("linux") {
        println!("cargo:rustc-link-arg=-Wl,-rpath,$ORIGIN/../lib/handy-hybrid:$ORIGIN/../lib");
    }
...
/// DLLs beside `handy-hybrid.exe` covers machines with no redistributable installed and
...
///
/// Where the staged dir lands: Windows bundles it beside `handy-hybrid.exe` (DLLs resolve
/// from the exe dir); Linux deb/rpm map it into the app-private `/usr/lib/handy-hybrid`
