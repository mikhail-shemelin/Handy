...
fn should_use_local_streaming(settings: &AppSettings, model_supports_streaming: bool) -> bool {
    !settings.uses_openai_transcription() && model_supports_streaming
}

fn remove_untracked_recording(path: &Path) {
    match std::fs::remove_file(path) {
        Ok(()) => debug!("Removed untracked recording: {}", path.display()),
        Err(err) if err.kind() == ErrorKind::NotFound => {}
        Err(err) => error!(
            "Failed to remove untracked recording {}: {}",
            path.display(),
            err
        ),
    }
...
impl ShortcutAction for TranscribeAction {
    fn start(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        // Load a local model in the background only for the local route.
        let tm = app.state::<Arc<TranscriptionManager>>();
        ...
        let model_supports_streaming = should_use_local_streaming(
            &settings,
            selected_model_info
                .as_ref()
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Some(samples) = rm.stop_recording(&binding_id, cancel_generation) {
                ...
                if samples.is_empty() {
                    ...
                    let file_name = format!("handy-hybrid-{}.wav", chrono::Utc::now().timestamp());
                    ...
                    let route_settings = get_settings(&ah);
                    let transcription_result = if route_settings.uses_openai_transcription() {
                        // The cloud route is completed-audio only. Dropping this
                        // future on cancellation also drops the in-flight HTTP
                        // request; no stream worker is ever started for it.
                        tm.cancel_stream();
                        complete_unless_cancelled(
                            tm.transcribe_completed_audio_for_settings(
                                samples,
                    ...
                    if !wav_saved {
                        // A failed or unverifiable save can still leave a partial
                        // file. Without a history row it would escape retention.
                        remove_untracked_recording(&wav_path_for_verify);
                    }

                    let Some(transcription_result) = transcription_result else {
                        debug!("OpenAI transcription operation cancelled");
                        remove_untracked_recording(&wav_path_for_verify);
                        utils::hide_recording_overlay(&ah);
                        change_tray_icon(&ah, TrayIconState::Idle);
                        return;
...
mod tests {
    use super::{
        complete_unless_cancelled, is_blank_transcription, remove_untracked_recording,
        should_use_local_streaming, should_use_streaming_overlay,
    };
    use crate::settings::{
        get_default_settings, OverlayStyle, TranscriptionProvider, OPENAI_TRANSCRIPTION_PROVIDER_ID,
    };
    use std::future;
    ...
    #[test]
    fn untracked_recording_cleanup_removes_file_and_tolerates_missing_file() {
        let temp_dir = tempfile::tempdir().unwrap();
        let path = temp_dir.path().join("cancelled.wav");
        std::fs::write(&path, b"partial wav").unwrap();

        remove_untracked_recording(&path);
        assert!(!path.exists());
    ...
    #[test]
    fn openai_route_never_uses_retained_local_streaming_capability() {
        ...
        assert!(!should_use_local_streaming(&settings, true));
