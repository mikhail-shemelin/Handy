...
fn apply_settings_migrations(
    ...
    // Early Hybrid builds persisted the active OpenAI provider before an
    // explicit enablement flag existed. Preserve that route unless a stored
    // flag explicitly disabled it.
    if settings_value.get("openai_transcription_enabled").is_none()
    ...
    if !settings
        ...
        .contains_key(OPENAI_TRANSCRIPTION_PROVIDER_ID)
    {
        settings
            .openai_transcription_api_keys
    ...
    // already made it through model selection. A fully configured active cloud
    // route also represents a completed choice even when no local model exists.
    ...
    if stored_schema_version < 1 {
        // Handy Hybrid 0.8.x used whisper_* names for the same user intent.
        // Migrate them only when the new fields are absent so a partially
        // upgraded store never overwrites newer values.
        if settings_value.get("transcribe_accelerator").is_none() {
            if let Some(old_accelerator) = settings_value.get("whisper_accelerator") {
                if let Ok(value) =
                    serde_json::from_value::<TranscribeAcceleratorSetting>(old_accelerator.clone())
                {
                    settings.transcribe_accelerator = value;
                }
            }
        }
        if settings_value.get("transcribe_gpu_device").is_none() {
            if let Some(old_device) = settings_value
                .get("whisper_gpu_device")
                .and_then(|value| value.as_i64())
                .and_then(|value| i32::try_from(value).ok())
            {
                settings.transcribe_gpu_device = old_device;
            }
        }
...
mod tests {
    ...
    #[test]
    fn legacy_hybrid_cloud_only_route_completes_onboarding() {
        let raw = serde_json::json!({
            "selected_model": "",
            "transcription_provider": "openai",
            "openai_transcription_base_url": "https://stt.example.test/v1",
            "openai_transcription_model": "gpt-4o-mini-transcribe",
            "openai_transcription_api_keys": {
                "openai": "sk-hybrid-fixture-DO-NOT-LOG"
            },
            "overlay_position": "none"
        });
    ...
    #[test]
    fn representative_v0_8_3_hybrid_store_migrates_without_losing_user_values() {
        // Representative of the settings object written by Handy Hybrid 0.8.3:
        // it predates the upstream schema/onboarding/theme/VAD keys and uses the
        // old whisper_* accelerator names, while already carrying Hybrid's
        // completed-audio OpenAI route.
        let raw = serde_json::json!({
            "bindings": {
                "transcribe": {
                    "id": "transcribe",
            ...
            "push_to_talk": false,
            "audio_feedback": true,
            "audio_feedback_volume": 0.65,
            "selected_model": "whisper-medium",
            "transcription_provider": "openai",
            "openai_transcription_enabled": true,
            "openai_transcription_base_url": "https://stt.example.test/v1",
            "openai_transcription_model": "gpt-4o-mini-transcribe",
            "openai_transcription_prompt": "Ada, Handy Hybrid",
            "openai_transcription_chunking_enabled": true,
            "openai_transcription_api_keys": {
                "openai": "sk-v083-fixture-DO-NOT-LOG"
            },
            "selected_microphone": "USB Test Microphone",
            "translate_to_english": true,
            "selected_language": "de",
            "overlay_position": "none",
            "custom_words": ["Ada", "Tauri"],
            "history_limit": 17,
            "paste_method": "direct",
            ...
            "post_process_provider_id": "openai",
            "post_process_api_keys": {
                "openai": "sk-postprocess-fixture-DO-NOT-LOG"
            },
            "post_process_models": {
                "openai": "gpt-4o-mini"
            },
        ...
        assert!(!settings.push_to_talk);
        assert!(settings.audio_feedback);
        assert_eq!(settings.audio_feedback_volume, 0.65);
        ...
        assert_eq!(
            settings
                ...
                .get("openai")
                .map(String::as_str),
            Some("sk-postprocess-fixture-DO-NOT-LOG")
        );
        assert_eq!(
            settings
                .post_process_models
        ...
        assert_eq!(settings.app_language, "de");
        assert!(!settings.show_tray_icon);
        assert_eq!(settings.paste_delay_ms, 85);
        assert_eq!(
            settings.transcribe_accelerator,
            TranscribeAcceleratorSetting::Cpu
        );
        assert_eq!(settings.transcribe_gpu_device, -1);
        assert_eq!(settings.ort_accelerator, OrtAcceleratorSetting::Cpu);
        assert_eq!(settings.extra_recording_buffer_ms, 240);

        // New upstream settings get their intended defaults/migrations without
        // forcing the legacy store back to a wholly fresh profile.
        assert_eq!(
            settings.settings_schema_version,
            CURRENT_SETTINGS_SCHEMA_VERSION
        );
    ...
    #[test]
    fn legacy_hybrid_accelerator_names_preserve_unambiguous_values() {
        let raw = serde_json::json!({
            "whisper_accelerator": "cpu",
            "whisper_gpu_device": -1
        });
