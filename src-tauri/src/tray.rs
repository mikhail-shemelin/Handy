...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("Handy Hybrid v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("Handy Hybrid v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
pub fn update_tray_menu(app: &AppHandle, locale: Option<&str>) {
    ...
    let model_submenu = {
        ...
        for model in &downloaded {
            let is_active = local_route_active && model.id == *current_model_id;
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "handy-hybrid-1.wav".to_string(),
            timestamp: 0,
            saved: false,
