...
impl<D> Drop for Stream<D> {
    fn drop(&mut self) {
        // Handy creates simple streams through pw_stream_new_simple. Callback
        // processing also creates temporary wrappers around that same pointer,
        // so restrict destruction to the outer simple-stream owner.
        if !matches!(&self._alive, KeepAlive::Simple { .. }) {
            return;
        }
...
mod ownership_tests {
    ...
    #[test]
    fn owned_simple_stream_drop_releases_native_resources() {
        crate::init();
        let initial_fds = process_resource_count("/proc/self/fd");
        let initial_data_loops = data_loop_count();

        for _ in 0..10 {
            let mainloop = crate::MainLoop::new().expect("create PipeWire main loop");
            let stream = Stream::<()>::simple(
                &mainloop,
                "pipewire-stream-drop-test",
                crate::properties! { "media.type" => "Audio" },
            )
            .create()
            .expect("create PipeWire simple stream");
            drop(stream);
        }

        assert_eq!(data_loop_count(), initial_data_loops);
